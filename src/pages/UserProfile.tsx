...
const UserProfile = () => {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      const hasAI = postContent.toLowerCase().includes('@ai');
      const postData: any = {
        type: postType,
        content: postType === 'text' ? postContent.trim() : '',
        imageUrl: postType === 'image' ? selectedImage : '',
      };
      if (hasAI) {
        const lines = postContent.trim().split('\n');
        postData.featureTag = 'ai';
        postData.featureData = {
          name: lines[0].replace(/@ai/gi, '').trim(),
          fullContent: lines.slice(1).join('\n').trim(),
        };
      }
      const res = await api.posts.createPost(postData);
      setUserPosts(prev => [res.post, ...prev]);
      setPostContent(""); setSelectedImage(null);
      setShowPostModal(false); setPostType('text');
    } catch {}
  };
                  ...
                  <div>
                    ...
                    {!post.featureTag && (
                      post.type === 'text'
                        ...
                        : <img src={post.imageUrl} alt="Post" className="up-image-preview" />
                    )}
                  </div>
                  <div className="up-post-actions">
                    <button className="up-action-btn"><Heart size={13} /> {post.likes || 0}</button>
                    <button className="up-action-btn"><MessageCircle size={13} /> {post.comments || 0}</button>
                    <div className="up-action-spacer" />
                    <button className="up-action-btn"><Bookmark size={13} /></button>
                  </div>
                </div>
              ))
            )}
            ...
            <div className="up-modal" onClick={e => e.stopPropagation()}>
              ...
              {postType === 'text' ? (
                ...
                <div>
                  ...
                  {selectedImage && <img src={selectedImage} alt="Preview" className="up-image-preview" />}
                </div>
              )}
